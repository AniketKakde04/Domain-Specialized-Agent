...
export const AgentBuilder = () => {
    ...
    try {
        // 1. Get the auth token to pass to the backend
        const { data: { session } } = await supabase.auth.getSession();
        const token = session?.access_token;
        ...
        // 2. Call the new Magic Build endpoint
        const res = await fetch('http://localhost:8000/api/magic-build', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        // 3. Update the React Flow canvas with the AI-generated nodes and edges
        ...
        console.error("Magic Build failed:", error);
        alert("Magic Build failed to generate the graph. Please try a different prompt.");
    } finally {
        setIsGenerating(false);
    }
};
                ...
                <div className={`${isSidebarOpen ? 'w-64' : 'w-0'} transition-all duration-300 bg-slate-900 border-r border-slate-800 flex flex-col z-20 shrink-0 overflow-y-auto`}>
                    <div className="p-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div draggable onDragStart={(e) => onDragStart(e, 'default', 'Agent Tools', '#ec4899', 'tool')} className="bg-pink-500/10 border border-pink-500/20 p-3 rounded-xl cursor-grab hover:bg-pink-500/20 transition-colors flex items-center gap-3 group">
                                <div className="bg-pink-500 p-1.5 rounded-lg"><Search className="w-4 h-4 text-white" /></div>
                                <span className="text-sm font-semibold text-pink-100 group-hover:text-white">Agent Tools</span>
                            </div>
                            ...
                            <div draggable onDragStart={(e) => onDragStart(e, 'default', 'Google Slides', '#10b981', 'google_slides')} className="bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-xl cursor-grab hover:bg-emerald-500/20 transition-colors flex items-center gap-3 group">
                                <div className="bg-emerald-500 p-1.5 rounded-lg"><Database className="w-4 h-4 text-white" /></div>
                                <span className="text-sm font-semibold text-emerald-100 group-hover:text-white">Google Slides</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {selectedNode && isSettingsOpen && (
                    <div className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col z-20 shrink-0 shadow-2xl">
                        ...
                        <div className="p-5 flex-1 overflow-y-auto space-y-6">
                            ...
                            {/* --- PPT WRITER SETTINGS --- */}
{selectedNode.data.backendType === 'ppt_writer' && (
    <div className="space-y-4">
        <div>
            ...
            <input
                ...
                value={selectedNode.data.fileName || 'presentation.pptx'}
                onChange={(e) => updateNodeData('fileName', e.target.value)}
            />
        </div>
        <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Slide Title</label>
            <input
                type="text"
                className="w-full text-sm border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                value={selectedNode.data.slideTitle || 'Generated Report'}
                onChange={(e) => updateNodeData('slideTitle', e.target.value)}
            />
        </div>
        <p className="text-xs text-gray-500">
            This will create a .pptx file. The content generated by the previous node will be placed in the slide body.
        </p>
    </div>
)}

{/* --- GOOGLE SLIDES SETTINGS --- */}
{selectedNode.data.backendType === 'google_slides' && (
    <div className="space-y-4">
        {/* We reuse your Google Connection UI here! */}
        <div className="p-4 border border-amber-200 bg-amber-50 rounded-lg">
            <h3 className="font-semibold text-amber-800 text-sm mb-1">Account Connection</h3>
            {googleConnected ? (
                <div className="flex items-center gap-2 text-amber-700 font-bold text-sm bg-white p-2 rounded border border-amber-100">
                ...
                <button 
                    ...
                    className="w-full bg-white border border-gray-300 text-gray-700 font-medium text-sm px-4 py-2 rounded shadow-sm hover:bg-gray-50 flex items-center justify-center gap-2 mt-2"
                >
                    Sign in with Google
                </button>
            )}
        ...
        <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">New Presentation Title</label>
            <input
                type="text"
                className="w-full text-sm border-gray-300 rounded-md focus:ring-amber-500 focus:border-amber-500"
                value={selectedNode.data.presentationTitle || 'AI Generated Presentation'}
                onChange={(e) => updateNodeData('presentationTitle', e.target.value)}
            />
        </div>
        <p className="text-xs text-gray-500">
            This will create a brand new Google Slide in your Drive.
        </p>
    </div>
)}
