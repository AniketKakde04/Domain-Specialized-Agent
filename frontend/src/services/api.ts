...
export const sendMessage = async (botId: string, question: string, image_base64?: string) => {
    const token = await getAuthToken();
    const response = await fetch(`${API_URL}/chat`, {
        ...
        body: JSON.stringify({ bot_id: botId, question, image_base64 }),
    });
...
export const sendPublicMessage = async (shareId: string, question: string, image_base64?: string) => {
    const response = await fetch(`${API_URL}/public/chat/${shareId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question, image_base64 }),
    });
    if (!response.ok) throw new Error('Failed to send message');
    return response.json();
};
