...
async def chat(request: ChatRequest):
    ...
    try:
        bot_response = client.table("bots").select("workflow_id, system_prompt").eq("id", request.bot_id).single().execute()
        if not bot_response.data:
            raise HTTPException(status_code=404, detail="Bot not found")
            
        workflow_id = bot_response.data.get("workflow_id")
        system_prompt = bot_response.data.get("system_prompt")
    except Exception as e:
    ...
    if workflow_id:
        ...
        if wf_response.data:
            ...
            try:
                result = await build_and_run_workflow(nodes, edges, request.question, user_id=workflow_user_id, image_base64=request.image_base64)
        ...
        answer = get_answer(request.bot_id, request.question, api_key, system_prompt, image_base64=request.image_base64)
...
async def public_chat(share_id: str, request: ChatRequest):
    ...
    if workflow_id:
        ...
        if wf_response.data:
            try:
                result = await build_and_run_workflow(wf_response.data['nodes'], wf_response.data['edges'], request.question, user_id=wf_response.data.get('user_id'), image_base64=request.image_base64)
        ...
        answer = get_answer(bot_id, request.question, api_key, image_base64=request.image_base64)
...
@app.post("/api/magic-build")
async def magic_build(request: Request, user: dict = Depends(verify_user)):
    data = await request.json()
    ...
    llm = ChatGoogleGenerativeAI(
        ...
        temperature=0 # 0 makes it follow the JSON rules strictly
    )

    # --- THE MAGIC PROMPT ---
    sys_prompt = """You are an Expert AI Workflow Architect for BotCraft Builder. 
    The user will describe an automation task. You must return ONLY a valid JSON object representing the workflow graph.
    
    CRITICAL GRAPH RULES:
    1. Node 1 MUST ALWAYS be 'input'.
    2. Node 2 MUST ALWAYS be 'agent'.
    3. Edge 1 MUST ALWAYS connect 'input' to 'agent' (source: "1", target: "2").
    4. You CANNOT connect 'input' directly to a tool or action node. It must go through the agent.
    5. Tools ('search') must come AFTER the 'agent'.
    6. Action nodes ('doc_writer', 'excel_writer', 'ppt_writer', 'google_slides', 'email') must come AFTER the agent or search tool.
    7. Build a single, continuous chain (e.g., 1->2, 2->3, 3->4).
    
    JSON Format Required:
    {
      "nodes": [
         {"id": "1", "position": {"x": 50, "y": 250}, "data": {"label": "Start", "backendType": "input", "userPrompt": "..."}, "type": "input", "style": {"background": "#10b981", "color": "white", "borderRadius": "8px", "padding": "10px", "width": 160, "textAlign": "center", "fontWeight": "bold"}}
      ],
      "edges": [
    ...
    Available Nodes (`backendType`):
    - `input`: (id: 1, Color: #10b981). Must include `userPrompt`.
    - `agent`: (id: 2, Color: #4f46e5). Must include `systemInstruction` tailored to the task.
    - `search`: Web search (Color: #ec4899).
    - `email`: Sends email (Color: #f59e0b).
    - `doc_writer`: Word doc (Color: #3b82f6). Must include `filename`.
    - `excel_writer`: Excel sheet (Color: #10b981). Must include `filename`.
    - `ppt_writer`: PowerPoint (Color: #f97316). Must include `fileName` and `slideTitle`.
    - `google_slides`: Google Slides (Color: #f59e0b). Must include `presentationTitle`.

    Layout Rules:
    - Node 'x' positions must increment by 250 (e.g., 50, 300, 550, 800).
    - Node 'y' positions must remain 250.
    - Edges must correctly link `source` and `target` IDs using the format "e{source}-{target}".
    - OUTPUT RAW JSON ONLY. No markdown block quotes.
    """

    messages = [
        SystemMessage(content=sys_prompt),
        HumanMessage(content=f"Build a workflow for this request: {prompt}")
    ]
