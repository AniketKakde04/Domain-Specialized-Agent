...
def weather_tool(location: str) -> str:
    """Fetch current weather for a specific location. Use this to give farmers weather advisories."""
    try:
        ...
        weather_resp = httpx.get(weather_url).json()
        current = weather_resp.get("current", {})
        
        return f"Weather for {location}: Temp {current.get('temperature_2m')}°C, Rain {current.get('precipitation')}mm, Humidity {current.get('relative_humidity_2m')}%, Wind {current.get('wind_speed_10m')}km/h."
    except Exception as e:
        return f"Error fetching weather: {str(e)}"

@tool
def mandi_tool(commodity: str) -> str:
    """Fetch current mandi (market) prices for an agricultural commodity. Use this to advise farmers on pricing."""
    prices = {
        "tomato": "₹20/kg - ₹30/kg", "onion": "₹40/kg - ₹55/kg", "potato": "₹15/kg - ₹25/kg",
        "wheat": "₹2200/quintal", "rice": "₹3500/quintal", "soyabean": "₹4200/quintal",
        "cotton": "₹6500/quintal", "sugarcane": "₹315/quintal", "tur": "₹8500/quintal",
        "chickpea": "₹5200/quintal", "fenugreek": "₹30/kg"
    }
    comm_lower = commodity.lower()
    for key, value in prices.items():
        if key in comm_lower or comm_lower in key:
            return f"Current average Mandi price for {key.capitalize()} is around {value} across major markets today."
    
    return f"Average Mandi price for {commodity} is not currently available in the real-time system."
...
def get_llm_node(system_instruction: str, user_template: str, bind_tools: bool = False, mcp_config: Dict = None):
    async def llm_node_func(state: AgentState):
        ...
        all_tools = []
        if bind_tools:
            all_tools.extend([tavily_tool, weather_tool, mandi_tool])
...
async def build_and_run_workflow(nodes_config: List[Dict], edges_config: List[Dict], request_initial_input: str, user_id: str = None, image_base64: str = None):
    ...
    for node in nodes_config:
        ...
        if backend_type == 'input':
            ...
            workflow.add_node(node_id, ToolNode([tavily_tool, weather_tool, mandi_tool]))
    ...
    # Multimodal support
    init_message_content = final_input
    if image_base64:
    ...
    try:
        final_state = await asyncio.wait_for(
            graph_app.ainvoke({
                "messages": [HumanMessage(content=init_message_content)],
